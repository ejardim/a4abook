...
The `residuals()` method computes these residuals and generate a object which can be plotted using a set of packed methods. The argument `type` will allow the user to chose which residuals will be computed. By default the method computes standardized residuals.
...
Figure \@ref(fig:res) shows a scatterplot of standardized residuals with a smoother to guide (or mis-guide ...) your visual analysis. Note that the standardization should produce normal residuals with variance=1, which means that most residual values should roughly be between $\sim -2$ and $\sim 2$.
...
The above plots can all be done grouping by year instead of ages, in which case it can help distinguish non-modeled structural year effects. The argument `by`, by default "ages", needs to be set as "year".
...
d_sy <- residuals(fit, ple4, ple4.indices, by='year')
plot(d_sy)
```
...
```{r, catchdiag, fig.cap="Diagnostics for age aggregated catch in weight", eval=FALSE, echo=FALSE}
