...
simulate performed on a fitted model returns a fitted model with the same structure as the original model, but with the model parameters replaced by `nsim` simulated parameters. The following code simulates the model 200 times
...
If interest is in the complete stock, then the `+` operator can be used to add the simulated quantities to the initial stock object, as shown below.
