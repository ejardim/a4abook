...
Missing observations are encoded as `NA` for example:
```{r missing obs}
fit <- sca(ple4, ple4.indices)

ple4_missing <- ple4
catch.n(ple4_missing)["1", "2017"] <- NA

fit_missing <- sca(ple4_missing, ple4.indices)

pred <- ple4 + simulate(fit, 100)
pred_missing <- ple4 + simulate(fit_missing, 100)

flqs <- FLQuants(window(catch.n(pred), 2000)["1",], window(catch.n(pred_missing)["1",], 2000))
