...
```{r}
fmod <- ~ te(age, year, k = c(3,20)) + s(year, k = 5, by = as.numeric(age==1))
fit2 <- sca(ple4, ple4.indices, fmod)
```
...
xyplot(data~year|factor(age), groups=qname, data=flqs, type="l",
       ...
       strip.background=list(col="gray90")), ylab="", as.table=TRUE, layout=c(5,2))
```
...
bwplot(data ~ qname | factor(year) + factor(age), data = as.data.frame(flqs), scales = "free", auto.key = T, ylab = "Catch at age", layout=c(7,2), as.table=TRUE, par.settings=list(box.rectangle = list(col = "black"), box.umbrella = list(col = "black"), plot.symbol = list(col = "black")))
```
...
bwplot(data ~ qname | factor(year) + factor(age), data = as.data.frame(flqs2), scales = "free", auto.key = T, ylab = "Catch at age", layout=c(7,2), as.table=TRUE, par.settings=list(box.rectangle = list(col = "black"), box.umbrella = list(col = "black"), plot.symbol = list(col = "black")))
```
