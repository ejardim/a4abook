...
- [+:] update the stock object with the fitted fishing mortalities, population abundance and catch in numbers at age; can be applied to `FLStocks` and `a4aFits` objects as well.
...
```{r, vmodimpact, fig.cap="Population estimates using two different variance models", echo=FALSE}
...
wireframe(predict(fit)$vmodel[[1]], zlab="F")
```


This model fits smoothers to different sets of ages.
...
```{r, danai01, echo=FALSE, fig.cap="Catch at age variance model with a year effect.", echo=FALSE}
wireframe(harvest(fit), zlab="F")
```
...
```{r, echo = FALSE, fig.cap="Stock estimates with missing observations."}
pred <- ple4 + simulate(fit, 1000)
pred_missing <- ple4 + simulate(fit_missing, 1000)
flqs <- FLQuants(base = catch.n(pred)[, "2013"], missing = catch.n(pred_missing)[, "2013"])
bwplot(data ~ qname | factor(age), data = as.data.frame(flqs), scales = "free", auto.key = T, ylab = "Catch at age in 2013")
```
