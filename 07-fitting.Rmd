...
### More models [TO BE MOVED TO ITS OWN SECTION?]
...
Missing observations are encoded as `NA`, and usually occure if there was no sampling for a year, or, since we model observations on the log scale, if the observation was zero. The `a4a` framework can deal with missing observations in the catch matrix and indices. The example below shows how to set up a model with missing observations in the catch matrix.

```{r missing obs}
fit <- sca(ple4, ple4.indices)

ple4_missing <- ple4
catch.n(ple4_missing)["1", "2013"] <- NA
...
flqs <- FLQuants(catch.n(pred)["1",], catch.n(pred_missing)["1",])

xyplot(data ~ year,
  groups = interaction(qname, iter), data = flqs, type = "l",
  auto.key = list(points = FALSE, lines = FALSE, draw = FALSE),
  par.settings = list(
    superpose.line = list(col = c("gray35", "black"), lty = c(2, 1), lwd = c(1, 1.5)),
    strip.background = list(col = "gray90")
  ), ylab = ""
)

flqs <- FLQuants(base = catch.n(pred)["1", "2013"], missing = catch.n(pred_missing)["1", "2013"])


bwplot(data ~ qname | factor(age), data = as.data.frame(flqs), scales = "free", auto.key = T, ylab = "Catch at age 1 in 2013")
