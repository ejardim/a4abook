...
However, in `sca` residuals are estimates of observation error (measurement noise) and model error (misspecification, omitted variables), which means, unless the model is well specified and not missing any relevant variables, it will be an overestimate of observation error.
...
```{r, preduncert, fig.cap="Prediction (pu), residual (ru) and estimation (eu) uncertainty"}
plot(FLStocks(pu=stk.pu, eu=stk.eu, ru=stk.ru))
```
